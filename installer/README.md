# ModReady All-in-One Installer

The v2.0 distribution: one download that bundles **BLSE + the four BUTR
dependency modules** (Bannerlord.Harmony, Bannerlord.UIExtenderEx,
Bannerlord.ButterLib, Bannerlord.MBOptionScreen) so a new user is modding-ready
in a single step (then they just add the content mods they want). The **ModReady
framework module itself is not bundled** — this installer is the dependency stack
only. It does **not** replace BLSE — it bundles it, with permission (BLSE is MIT,
© BUTR; see licensing below).

## What the installer does

- Auto-detects the Bannerlord install (Steam registry → common path → user browse),
  and validates the chosen folder actually contains `bin\Win64_Shipping_Client\Bannerlord.exe`.
- Copies the four BUTR dependency module folders into `Modules\` (no ModReady folder).
- Copies BLSE's `bin\Win64_Shipping_Client\*` into the game's `bin\Win64_Shipping_Client\`
  (the documented BLSE install location).
- Ships all license notices into `Modules\Bannerlord.Harmony\licenses\` (the
  ModReady folder isn't shipped, so notices live under the Harmony dependency).
- Optional desktop shortcut to `Bannerlord.BLSE.LauncherEx.exe`, and an offer to
  launch it on finish.

## Files here

- `ModReady-AllInOne.iss` — the Inno Setup script (the installer definition).
- `Build-Installer.ps1` — stages the payload and compiles the `.iss` into a `.exe`.
- `payload\` — generated by the build script (git-ignore it); not committed.

## How to build it

One-time prerequisites:

1. Install **Inno Setup 6** (https://jrsoftware.org/isdl.php) — gives you `ISCC.exe`.
2. Download **BLSE** (Manual file) from Nexus mod 1 or the BUTR/Bannerlord.BLSE
   releases, and unzip it somewhere. The unzipped root must contain
   `bin\Win64_Shipping_Client\Bannerlord.BLSE.LauncherEx.exe` and a `LICENSE` file.
3. Make sure `dist\Modules` exists (run `.\scripts\Build-Phase1.ps1` first if not).

Then build:

```
cd C:\dev\modready\framework
.\installer\Build-Installer.ps1 -BlseDir "C:\path\to\unzipped-BLSE" -Version 1.0.0
```

Output: `dist\ModReady-AllInOne-v1.0.0.exe`.

## Licensing / credit

BLSE is redistributed under its **MIT license** (© 2021-2022 BUTR), which permits
bundling provided its copyright + permission notice ships with the bundle — the
build script copies BLSE's `LICENSE` into `payload\LICENSES\` and the installer
lays it down under `Modules\Bannerlord.Harmony\licenses\`. The Nexus page and in-app credits
acknowledge BUTR as the creators of the BUTR stack and BLSE. Verified MIT on
2026-06-08 — see the vault page **All-in-One ModReady Installer** for the full note.

## Open items before shipping

- Keep the bundled BLSE version current as BLSE updates.
- Give BUTR a courtesy heads-up that the bundle redistributes BLSE.
- Decide on code-signing the installer `.exe` (avoids SmartScreen warnings).
- Distribute on Nexus (not Steam Workshop — Workshop hosts modules only).
