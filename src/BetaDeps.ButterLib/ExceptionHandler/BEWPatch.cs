// BetaDeps clean-room re-implementation of ButterLib's BEWPatch.
...
public static class BEWPatch
{
    ...
    private const string HarmonyId = "betadeps.butterlib.bewpatch";
    ...
    private static void TryBind(HarmonyLib.Harmony harmony, string typeName, string methodName, string label)
    {
        try
        {
            var t = BetaDeps.Foundation.ReflectionUtils.ResolveTypeByFullName(typeName);
            if (t == null)
            {
