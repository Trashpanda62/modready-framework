@echo off
REM ============================================================
REM  ModReady -- Reset State
REM
REM  Run this if ModReady has gotten itself stuck (most common
REM  symptom: the "Mod Configuration" tab is missing from the
REM  in-game Options screen).
REM
REM  This wipes the state files ModReady writes between sessions:
REM    - modready-disabled-mods.log  (the crash-suspect block list)
...
REM    - runtime.log + archives      (ModReady' own diagnostic log)
...
REM
REM  Equivalent to "uninstall and reinstall ModReady" without
REM  re-downloading the zip.
REM ============================================================

setlocal
set "TARGET=%~dp0"
echo ModReady Reset State
echo Target: %TARGET%
echo.

choice /C YN /N /M "This will delete ModReady state files in the folder above. Proceed? (Y/N): "
if errorlevel 2 (
    echo Cancelled.
    pause
    exit /b 1
...
del /Q "%TARGET%modready-disabled-mods.log"     2>nul
...
del /Q "%TARGET%modready-compat-warnings.log"   2>nul
...
REM M5 (Phase 4.2): remove the persistent User-scope env var ModReady sets
REM so BLSE's launcher shows the dependency modules. Harmless if absent;
REM ModReady re-creates it on the next launch if still installed.
...
echo If it isn't, drag the new runtime.log to the ModReady Nexus posts page.
pause
