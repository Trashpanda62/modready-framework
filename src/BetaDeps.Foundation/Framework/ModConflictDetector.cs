...
//
// ModConflictDetector -- BetaDeps v2.0 framework primitive #2.
//
...
// touching it. A method touched by >= 2 non-BetaDeps owners is a reported
...
//
// BetaDeps's own owners are excluded by default: PatchShield attaches a
...
using BetaDeps.Foundation;   // DiagLog
...
namespace BetaDeps.Framework
{
    ...
    {
        private const string Tag = "BetaDeps.ConflictDetector";

        /// <summary>
        /// Scan Harmony's global registry for methods patched by two or more
        /// distinct owners. Excludes BetaDeps's own owners by default (its
        /// shield finalizers touch everything). Results are ordered most-severe
        /// first, then by signature for stable output.
        /// </summary>
        /// <param name="includeBetaDeps">
        /// When true, BetaDeps owners are counted too (diagnostic use only).
        /// </param>
        public static IReadOnlyList<MethodConflict> Scan(bool includeBetaDeps = false)
        {
            ...
            foreach (var method in methods)
            {
                ...
                MethodConflict? c;
                try { c = Evaluate(method, includeBetaDeps); }
                catch (Exception ex)
                {
        ...
        private static MethodConflict? Evaluate(MethodBase method, bool includeBetaDeps)
        {
            ...
            void Fold(IEnumerable<Patch>? patches, PatchKinds kind)
            {
                ...
                foreach (var p in patches)
                {
                    ...
                    if (!includeBetaDeps && IsBetaDepsOwner(owner, p)) continue;
        ...
        private static bool IsBetaDepsOwner(string owner, Patch p)
        {
            if (owner.StartsWith("BetaDeps", StringComparison.OrdinalIgnoreCase)) return true;
            // Also catch patches whose method lives in a BetaDeps assembly even
            // if the owner id was set to something else.
            var asm = AssemblyOf(p);
            return asm.StartsWith("BetaDeps", StringComparison.OrdinalIgnoreCase);
        }
        ...
        public static string ToText(IReadOnlyList<MethodConflict>? conflicts = null)
        {
            ...
            sb.AppendLine($"BetaDeps mod-conflict report -- {conflicts.Count} contested method(s)");
        ...
        public static string ToMarkdown(IReadOnlyList<MethodConflict>? conflicts = null)
        {
            conflicts ??= Scan();
            var sb = new StringBuilder();
            sb.AppendLine("### BetaDeps mod-conflict report");
            sb.AppendLine();
