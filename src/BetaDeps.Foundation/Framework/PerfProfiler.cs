...
//
// PerfProfiler -- BetaDeps v2.0 framework primitive #3.
//
...
namespace BetaDeps.Framework
{
    ...
    {
        private const string Tag = "BetaDeps.PerfProfiler";
        private const string HarmonyId = "BetaDeps.Foundation.PerfProfiler";
        ...
        public static string ToText(int top = 25)
        {
            ...
            sb.AppendLine($"BetaDeps perf report -- {snap.Count} bucket(s), top {Math.Min(top, snap.Count)} by total time:");
        ...
        public static int Instrument(IEnumerable<MethodBase> methods)
        {
            ...
            lock (_instrumentLock)
            {
                foreach (var m in methods)
                {
                    ...
                    // Don't instrument BetaDeps' own methods -- it would charge
                    // our shields' overhead back to us and add noise.
                    try
                    {
                        var asm = m.DeclaringType?.Assembly?.GetName()?.Name ?? "";
                        if (asm.StartsWith("BetaDeps", StringComparison.OrdinalIgnoreCase)) continue;
                    }
                    catch { }
...
#pragma warning restore IDE0051, IDE1006
        ...
        private static string[] OwnersOf(MethodBase m)
        {
            try
            {
                ...
                return info.Owners
                    .Where(o => !string.IsNullOrEmpty(o) && !o.StartsWith("BetaDeps", StringComparison.OrdinalIgnoreCase))
