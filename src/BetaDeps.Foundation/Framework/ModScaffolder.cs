...
//
// ModScaffolder -- BetaDeps v1.x modder-layer "new-mod" generator. Produces a
// ready-to-build starter mod that consumes BetaDeps, so an author isn't staring
...
//                   applies a Harmony patch via BetaDeps.Harmony's SafeBind and
...
// contents. A thin `betadeps new-mod` CLI can wrap Generate() later.
...
namespace BetaDeps.Framework
{
    ...
    {
        private const string Tag = "BetaDeps.ModScaffolder";
        ...
        {
            // Declares BetaDeps as a dependency so the launcher loads it first.
...
";
  ...
  <ItemGroup>
    ...
    <!-- BetaDeps assemblies are provided at runtime by the BetaDeps module.
...
";
        ...
        private static string SubModuleCs(ScaffoldOptions o, string id, string name)
        {
            string settingsLine = o.Template == ModTemplate.Full
                ...
                : "";
            return $@"// {name} -- generated by BetaDeps ModScaffolder.
using System;
using HarmonyLib;
using TaleWorlds.MountAndBlade;
using BetaDeps.Foundation;   // DiagLog
using BetaDeps.Harmony;       // SafeBind
...
{{
    ...
    {{
        ...
        {{
            ...
            // Example: signature-safe Harmony patch via BetaDeps. SafeBind
...
";
        ...
        private static string SettingsCs(string id, string name) => $@"// {name} settings -- generated by BetaDeps ModScaffolder.
...
";
        ...
        private static string Readme(ScaffoldOptions o, string id, string name)
        {
            ...
            sb.AppendLine($"# {name}");
            sb.AppendLine();
            sb.AppendLine($"A Bannerlord mod scaffolded by BetaDeps ({o.Template}). Author: {o.Author}.");
            sb.AppendLine();
            sb.AppendLine("## Install (for testing)");
            sb.AppendLine($"Copy this `{id}` folder into your Bannerlord `Modules\\` folder, enable");
            sb.AppendLine("**BetaDeps** and this mod in the launcher, and launch.");
            sb.AppendLine();
            if (o.Template == ModTemplate.SettingsOnly)
            {
                sb.AppendLine("## Editing settings");
                sb.AppendLine("This mod has no code — edit `mod.json` to add/remove settings. BetaDeps");
