...
internal static class ViewModelMixinHook
{
    ...
    private const string HarmonyId = "modready.uiextenderex.vmmixin";
        ...
        foreach (var mixinReg in registry.Mixins)
        {
            try
            {
                ...
                var targetType = !string.IsNullOrEmpty(mixinReg.Attribute.TargetTypeName)
                    ? ModReady.Foundation.ReflectionUtils.ResolveTypeByFullName(mixinReg.Attribute.TargetTypeName!)
