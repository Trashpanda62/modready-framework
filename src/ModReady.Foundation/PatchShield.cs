...
{
    private const string Tag = "ModReady.PatchShield";
    private const string HarmonyId = "ModReady.Foundation.PatchShield";
    ...
    /// patchshield-disabled.flag in Modules\ModReady\).
    /// </summary>
                ...
                foreach (var method in targets)
                {
                    ...
                    try
                    {
                        ...
                        if (declaring != null)
                        {
                            ...
                            if (asmName.StartsWith("ModReady", StringComparison.OrdinalIgnoreCase))
                            {
    ...
    // The v5 behavior stripped every non-ModReady prefix on the method, so
    ...
    private static void TryUnpatchOffendingPatchesLocked(MethodBase originalMethod, CulpritFinder.CulpritInfo culprit, string sigKey)
    {
        try
        {
            ...
            foreach (var owner in culpritOwners)
            {
                ...
                if (owner.StartsWith("ModReady", StringComparison.OrdinalIgnoreCase))
                {
                    DiagLog.Log(Tag, $"refusing to unpatch ModReady owner '{owner}' on {sigKey}");
                    continue;
                }
