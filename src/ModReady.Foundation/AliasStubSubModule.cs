...
// AssemblyResolve hook on first invocation. Whichever module (ModReady or
...
public class AliasStubSubModule : MBSubModuleBase
{
    ...
    // this hook lands BEFORE BK's class is even instantiated (ModReady's
    ...
    public AliasStubSubModule()
    {
        if (System.Threading.Interlocked.Exchange(ref _earlyDetectionRan, 1) == 0)
        {
            ...
            // ModReadyHarmonySubModule's ctor also installs both, whichever
            // runs first wins.
            try
            {
            ...
            // AssemblyLoaderDependencyShim disabled -- see ModReadyHarmonySubModule for rationale.
