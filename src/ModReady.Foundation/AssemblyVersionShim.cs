...
// load: AliasStubSubModule (ctor + OnSubModuleLoad) and ModReadyHarmonySubModule
...
public static class AssemblyVersionShim
{
    ...
    private static Assembly? OnAssemblyResolve(object sender, ResolveEventArgs args)
    {
        try
        {
            ...
            // PREFER the assembly loaded from the ModReady-managed module tree, and
            ...
            if (matches.Count > 0)
            {
                var match = matches.FirstOrDefault(a => IsModReadyManagedDir(SafeDir(a))) ?? matches[0];
                if (matches.Count > 1)
                    DiagLog.Log(Tag, $"redirect {requested.Name} v{requested.Version} -> {matches.Count} loaded copies; chose v{match.GetName().Version} from '{SafeDir(match)}' (ModReady-managed preferred)");
                else
    ...
    // Mirror of HarmonyRuntimeGate.IsModReadyManagedDir -- duplicated here because
    // Foundation is the lowest assembly in the stack and cannot reference
    // ModReady.Harmony. KEEP THE TWO COPIES IN SYNC (the `managed` list below and
    // HarmonyRuntimeGate.cs's must match). True when the directory belongs to the
    // ModReady-managed module tree (Modules\ModReady\bin\... or one of the four
    // dependency modules).
    private static bool IsModReadyManagedDir(string dir)
    {
        ...
        string[] managed = {
            // Retained even though the ModReady installer ships no ModReady folder:
            // a user who ALSO installs the standalone ModReady framework mod will
            // have Modules\ModReady\, and its Foundation copy is equally canonical.
            "\\Modules\\ModReady\\",
