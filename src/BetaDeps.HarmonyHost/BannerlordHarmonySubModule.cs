// BetaDeps v0.8 step 1: real Bannerlord.Harmony module host.
//
// MIT, copyright 2026 Maxfield Management Group.
//
// PURPOSE
//   This SubModule sits inside Modules\Bannerlord.Harmony — promoted from an
//   alias-stub (which previously pointed at BetaDeps.Foundation.AliasStubSubModule)
...
//
//   By the time the engine resolves Modules\BetaDeps\SubModule.xml, this
//   module has ALREADY loaded — BetaDeps's SubModule.xml carries a
//   <DependedModule Id="Bannerlord.Harmony" /> entry that pins the load order.
//   That means BetaDeps's own SubModule classes JIT against the SAME Harmony
...
//   ButterLib DI, UIExtenderEx — all of that stays in Modules\BetaDeps\.
...
//   fired (and at what timestamp relative to BetaDeps's own OnSubModuleLoad).
//   Any exception is swallowed and logged — a logging failure in the host
//   must not block the engine from continuing to load BetaDeps.
    ...
    protected override void OnSubModuleLoad()
    {
        try
        {
            ...
            DiagLog.Log(Tag,
                ...
                "Harmony binary fan is now in the AppDomain before BetaDeps's SubModule constructs.");
        }
