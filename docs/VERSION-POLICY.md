# BetaDeps version policy (M19 / Phase 4.1, 2026-06-11)

BetaDeps ships two kinds of assemblies, and they version differently on
...
actual BetaDeps build that produced the DLL goes in
**InformationalVersion** (`BetaDeps <release>`), which is where diagnostics
should look.
...
`Bannerlord.Harmony.dll` (BetaDeps.HarmonyHost) is a half-exception:
...
FileVersion tracks the actual BetaDeps release (e.g. 0.9.2.0) and is the
number to read when checking what's deployed. The BetaDeps module's own
`SubModule.xml` version (`v0.9.2`) tracks the release too.

## Release checklist touchpoints

- Bump: BetaDeps module `SubModule.xml` version + internal assemblies'
