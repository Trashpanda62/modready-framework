# ModReady Direct-API Guide

**Audience:** Bannerlord mod authors who want to consume ModReady directly
...
- The four alias modules exist for backward compatibility — they let mods
  built against the original BUTR stack load against ModReady without
  recompilation. Eventually they go away.
- Code written against the `ModReady.*` namespaces gets stable surfaces
  that don't have to fingerprint upstream BUTR for ABI compatibility.
- You get access to ModReady-native conveniences (PatchShield install
  hooks, `SafeBind`, `DiagLog`) that the alias surface intentionally
  doesn't expose.

**Status as of v1.0.0:** the BUTR-impersonation surface and the
ModReady-native surface co-exist. v1.0.0 adds the **framework-core**
primitives (EventBus, ModConflictDetector, ProfileManager, PerfProfiler) —
see **[Module 7: ModReady.Framework](#module-7-modreadyframework-v10)** below.
...
- **v0.8 — the four aliases are now real standalone modules.**
  ...
  load (in dependency order) before `ModReady`. They're also available as
  individual optional downloads. This doesn't change how you reference
  ModReady — a consumer mod still lists `ModReady` (or any of the four) as
...
A ModReady installation under `Modules\ModReady\` ships these assemblies:

| Assembly                | Purpose                                              |
|-------------------------|------------------------------------------------------|
| `ModReady.Foundation`   | Logging, version probing, PatchShield, reflection    |
| `ModReady.Harmony`      | Harmony 2.x re-export + SafeBind sigsafe patcher     |
| `ModReady.UIExtenderEx` | UIExtender, prefab patches, view-model mixins        |
| `ModReady.ButterLib`    | HotKeys, sub-systems, exception handler, MBEvents    |
| `ModReady.MCM`          | Settings (Global / PerCampaign / PerSave), fluent builder |
| `ModReady.Module`       | SubModule entry, in-game Mod Config screen           |

Every consumer mod that needs any of the above lists `ModReady` as a
...
## Module 1: ModReady.Foundation

Diagnostic + bootstrap layer. Every ModReady-native consumer touches this.
...
`DiagLog.Log(tag, message)` writes to `Modules\ModReady\runtime.log`.
...
`Modules\ModReady\patchshield-disabled.flag` file (or click `Toggle
...
`saveshield-swallow-disabled.flag` in `Modules\ModReady\`). Programmatic
read/write:
...
Append-only ledger at `Modules\ModReady\failed-mods-catalog.txt`. Each
...
`0Harmony.dll`. The ModReady-native addition is `SafeBind`.
...
## Module 3: ModReady.UIExtenderEx

Public API surface lives in `Bannerlord.UIExtenderEx.*` for backward
compatibility with consumer mods that already reference it. The ModReady
implementation is in `ModReady.UIExtenderEx.dll` — both the alias and the
...
There is no ModReady-native rename of these — the upstream surface
already lives in our codebase. The `ModReady.*` namespace doesn't add
anything you'd want here.

---

## Module 4: ModReady.ButterLib
...
## Module 6: ModReady.Module — the in-game Mod Config screen
...
files under `Modules\ModReady\`. If you ship a similar opt-in feature for
your own mod, mimic that pattern (flag file + UI toggle that creates or
deletes it).

---

## Module 7: ModReady.Framework (v1.0)

The v1.0 framework-core primitives. All live under the `ModReady.Framework`
namespace; `EventBus`, `ModConflictDetector`, `PerfProfiler`, and
`SettingsProfileStore` ship in `ModReady.Foundation.dll`, and `ProfileManager`
ships in `MCMv5.dll` — but you reach the whole surface with one
`using ModReady.Framework;` (referencing `ModReady` covers it). These are pull
APIs: nothing happens until you call them (the one exception is the auto
conflict scan, which ModReady runs for you).
...
mods (ModReady's own shields are excluded). ModReady auto-runs a scan at the
...
`Modules\ModReady\` and ModReady wraps every Harmony-patched method with timing,
...
all. Drop a **`mod.json`** in your module folder (or any `*.modready.json`) and
ModReady builds an MCM settings page from it at load — persistence, the Mod
...
use, `ModReady.Framework.ModJsonParser.Parse(json)` (pure → schema) and
...
`ModReady.Framework.ModScaffolder.Generate(options, targetRoot)` writes a
ready-to-build starter mod that consumes ModReady, so you don't hand-author the
...
Every template declares `<DependedModule Id="ModReady" />` so the launcher load
order is correct, and `Generate` returns the list of files it wrote.

---

## ModReady-native vs BUTR-compat migration table

| Want to do                          | BUTR-compat (works today)                         | ModReady-native (forward-looking)        |
|-------------------------------------|---------------------------------------------------|------------------------------------------|
...
| Define per-save settings            | `PerSaveSettings<TSelf>`                          | (same — ModReady added this in v0.7)     |
| Build settings imperatively         | `BaseSettingsBuilder.Create(...)`                 | (same — ModReady added `CreatePreset` + `SetGroupOrder` in v0.7) |
| Catch sloppy Harmony patches        | n/a (manual try/catch wrapping)                   | (automatic via PatchShield — no opt-in)  |
| Disable a known-broken mod          | n/a (write LauncherData.xml yourself)             | Drop module into `modready-disabled-mods.log` |
...
forever; "ModReady-native" just means importing from the ModReady
...
**SubModule.xml load order.** ModReady must load before your mod. Either
list `<DependedModule Id="ModReady" />` in your `<DependedModules>` (the
launcher then guarantees order), or set `ModReady` in your
`<ModulesToLoadAfterThis>` from the ModReady side. The launcher silently
ignores unknown IDs, so listing optional dependencies is safe.

**The `AssemblyResolve` hook.** ModReady installs an `AssemblyResolve`
hook at `[ModuleInitializer]` time so the first
`MCMv5.dll` / `0Harmony.dll` / `UIExtenderEx.dll` / `ButterLib.dll` the
runtime sees is ModReady's. If your mod bundles its own copy of any of
...
- `C:\dev\modready\framework\src\ModReady.Module\` — the ModReady own Mod Config
...
- **Bannerwar** (`C:\dev\bannerwar`) — standalone RTS-style battle command
  mod that consumes ModReady end-to-end: `DiagLog`, `SafeBind`, raw
  Harmony patches via `ModReady.Harmony`, and (soon) MCM settings + a
  ButterLib hotkey category for free-cam / time-control rebinds. The
  canonical "framework consumer" example.

---

## Debugging your mod against ModReady
...
ecosystem has shipped for years, but without ModReady's defensive
finalizers or Mod Configuration surface — disable the `ModReady`
...
ModReady once you want the safety net back.

### Disabling specific defensive layers while leaving ModReady enabled

When you're iterating on your own Harmony patches or save-load code,
ModReady's defensive layers will catch exceptions and silently unpatch
...
ModReady supports flag files for this. Drop an empty (or any-content)
file into `Modules\ModReady\` and the corresponding subsystem disables
itself on the next game launch. Delete the file to re-enable.

| Flag file (in `Modules\ModReady\`)    | Effect when present                                                                                                                                                                |
|----------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
...
| `auto-disable-enabled.flag`            | OPT-IN. Enables ModReady's predictive incompatible-mod scan and runtime crash-recovery auto-disable. Default behavior (no flag) leaves your mod list alone.                       |
...
**Why these aren't UI buttons.** Pre-v0.8 ModReady surfaced these as
...
- **Tactics editor** — a no-code way to author tactic mods. Substrate
  TBD (either a minimal in-ModReady view, or built atop Bannerwar as a
  sister mod).
- **Build-your-first-tactic walkthrough** — a step-by-step that produces
  a Nexus-shippable tactic package using ModReady-native APIs.

Bannerwar ships separately (`C:\dev\bannerwar`) as a standalone Nexus
mod that demonstrates consuming ModReady as a framework.
