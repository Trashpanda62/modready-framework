# Phase 1 — BetaDeps.Harmony rewrite scope
...
That's it. The rest of what BUTR's wrapper ships (`BannerlordHarmonyLoader.dll`, etc.) is plumbing to satisfy *its* update mechanism and is not required for BetaDeps.

## What we ship

```
Modules/BetaDeps/
  SubModule.xml                     (top-level, registers BetaDeps submodules)
  bin/Win64_Shipping_Client/
    0Harmony.dll                    (MIT, pardeike — unchanged NuGet artifact)
    BetaDeps.Harmony.dll            (NEW — our wrapper)
    BetaDeps.Foundation.dll         (NEW — log/version probe)
    Bannerlord.Harmony.dll          (NEW — type-forwarder shim for legacy consumers)
```

## BetaDeps.Harmony.dll — file inventory

```
src/BetaDeps.Harmony/
  BetaDeps.Harmony.csproj
  BetaDepsHarmonySubModule.cs       # MBSubModuleBase entry point
...
1. Build `BetaDeps.Harmony.dll` against .NET 4.7.2 + Bannerlord game refs.
2. Drop the four DLLs above into a fresh `Modules\BetaDeps\` and enable in launcher.
...
6. `Modules\BetaDeps\runtime.log` contains "BetaDeps.Harmony loaded" and the sigsafe pre-bind verification lines.
...
- **Reference but rewrite:** the sigsafe-binding *concept* from CREST's `CrestBattleSizeBetaPatches.cs` — this is our own original work from the CREST sprint, so re-authoring it under BetaDeps names is paperwork, not infringement.
