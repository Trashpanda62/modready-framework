# ModReady rebuild plan

## Why we're doing this

1. **BUTR upstream is broken on beta (e1.4.x).** That's why CREST was built. ModReady inherits CREST's beta fixes as its starting baseline.
...
- New project skeleton under `modready/`.
...
### Phase 1 — ModReady.Harmony (clean-room wrapper rewrite)
...
Scope:
- `ModReady.Harmony.csproj` — references `Lib.Harmony` NuGet directly, plus TaleWorlds refs.
- `ModReadyHarmonySubModule.cs` — derives from `MBSubModuleBase`; on `OnSubModuleLoad` ensures Lib.Harmony is loaded before any patch-using sibling submodule.
- `ModReadyHarmonyTypeForwards.cs` — type-forwards `HarmonyLib.*` from `0Harmony.dll` so consumer mods compiled against standard Harmony resolve transparently.
...
- Module folder name: `ModReady.Harmony` during dev; the final ship layout is a single `Modules\ModReady\`.

Acceptance:
- Launches on e1.4.x beta and e1.3.x public without CTD.
- Other ModReady / consumer-mod patches bind via the wrapper.
- DLL inspector shows no Aragas copyright lines and no copied source blocks.

### Phase 2 — ModReady.UIExtenderEx (clean-room rewrite)
...
- Hook into `GauntletMovie.Load` via ModReady.Harmony.
...
### Phase 3 — ModReady.ButterLib and ModReady.MCM
...
Until Phase 3 lands, ModReady ships with `bundled-deps/ModReady.ButterLib/` and `bundled-deps/ModReady.MCM/` rebranded from CREST as a transitional measure. The third-party license file documents that these are derivative works of BUTR's MIT-licensed sources.

### Phase 4 — Single-module packaging

- One `SubModule.xml` at `Modules\ModReady\` referencing every assembly above.
- One Nexus zip: `ModReady.zip`.
...
We use `ModReady.<Component>` for nested project folders during development but the final ship layout is a single `Modules\ModReady\` on disk. This avoids the "10 standalone modules in the launcher" UX problem.

## License posture

- ModReady original code: MIT, copyright Steve / Maxfield Management Group.
...
- **No Aragas-authored code is shipped.** The build verifier checks every ModReady-authored DLL for the string "Aragas" and fails the build if found.
- **Namespace strings like `Bannerlord.BUTR`, `HarmonyLib.BUTR.Extensions` ARE used inside ModReady**, by design. These are public-API namespace paths the BUTR community standardized on; consumer mods compiled against those namespaces resolve drop-in to ModReady's clean-room implementations. Using a namespace name is not the same as importing copyrighted code.
